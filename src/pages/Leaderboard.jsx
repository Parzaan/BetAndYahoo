...
export default function LeaderboardPage() {
  ...
  return (
    <div className="p-8 w-full max-w-6xl mx-auto min-h-screen bg-market-bg text-market-text">
          ...
          {['weekly', 'monthly', 'all'].map(t => (
            <button
              ...
              className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${
                timeframe === t 
                  ? 'bg-market-accent text-market-bg shadow-lg' 
          ...
          return (
            <motion.div
              ...
              className={`relative flex flex-col items-center p-8 rounded-3xl border transition-all duration-500 ${
                isFirst 
                  ? 'bg-market-sidebar/40 border-market-accent/40 shadow-[0_0_50px_-12px_rgba(191,177,193,0.3)] order-1 md:order-2 md:h-80' 
                  : 'bg-market-sidebar/20 border-market-border/10 order-2 md:order-1 md:h-65'
              } ${i === 2 ? 'md:order-3' : ''}`}
            >
              {isFirst && (
                <div className="absolute -top-6 left-1/2 -translate-x-1/2">
                  ...
                  >
                    <Crown className="h-12 w-12 text-yellow-400 fill-yellow-400/20" />
                  </motion.div>
                </div>
              )}
          ...
          {leaderboard.map((player, i) => (
            ...
            >
              <div className="col-span-2 flex items-center gap-4 px-4">
                ...
                <span className={`font-medium ${player.isUser ? 'text-market-accent font-bold' : ''}`}>
                  {player.username}
